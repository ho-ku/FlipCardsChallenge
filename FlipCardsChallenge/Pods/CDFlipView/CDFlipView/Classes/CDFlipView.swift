...
open class CDFlipView: UIView {
    ...
    fileprivate func rotateToSideAtIndex(_ index:Int, duration:Double, goingIn:Bool, delay:TimeInterval){
        ...
        if goingIn{
            UIView.animate(withDuration: duration/2, delay: delay, options: UIView.AnimationOptions.curveLinear, animations: { () -> Void in
                self.multipleLayerView?.alpha = 1
            }) { (complete) -> Void in
            }
        }
        else{
            UIView.animate(withDuration: duration/2, delay: delay + duration/2, options: UIView.AnimationOptions.curveLinear, animations: { () -> Void in
                self.multipleLayerView?.alpha = 0
            }) { (complete) -> Void in
            }
        }
        
        UIView.animate(withDuration: duration, delay: delay, options: UIView.AnimationOptions.curveLinear, animations: { () -> Void in
